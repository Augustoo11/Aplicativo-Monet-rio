...
export default function Home() {
  ...
  // Ao trocar o tipo, limpa campos específicos
  useEffect(() => {
  ...
  async function buscarTransacoes(idDoUsuario: string) {
    try {
      ...
      if (resposta.ok) {
        ...
        setTransacoesDoBanco(dados.map((t: any) => ({
          id: String(t.id),
          tipo: t.tipo,
          valor: t.valor,
  ...
  async function buscarMetasDisponiveis(idDoUsuario: string) {
    try {
      ...
      if (resposta.ok) {
        const dados = await resposta.json();
        setMetasDisponiveis(dados.map((m: any) => ({
          id: m.id,
          nome: m.nome,
  ...
  async function salvarTransacao() {
    if (!categoriaSelecionada) { Alert.alert('Atenção', 'Selecione uma categoria.'); return; }
    if (!valor) { Alert.alert('Atenção', 'Informe o valor.'); return; }
    if (!usuarioId) { Alert.alert('Erro', 'Usuário não identificado.'); return; }
    ...
    if (valorNumerico <= 0) { Alert.alert('Atenção', 'Digite um valor válido.'); return; }
    ...
    } catch { Alert.alert('Erro de conexão', 'Verifique o servidor.'); }
  ...
  function limparFormulario() {
    setValor(''); setDescricao(''); setCategoriaSelecionada(null);
    setTipo('despesa'); setEhDespesaMeta(false); setMetaSelecionada(null);
    setModalTransacao(false);
  }

  // Formata enquanto digita: "15000" → "R$ 150,00"
  ...
  const iniciais = nomeUsuario.split(' ').filter(Boolean).slice(0, 2).map((n) => n[0].toUpperCase()).join('');
    ...
    <SafeAreaView style={estilosHome.container}>
      <View style={estilosHome.conteudo}>
        ...
        <View style={estilosHome.cabecalho}>
          <View style={estilosHome.logoRow}>
            <View style={estilosHome.logoBox}>
              <Image source={require('../../src/assets/logo.png')} style={estilosHome.logoImagem} resizeMode="contain" />
            </View>
            <Text style={estilosHome.logoTexto}>GestorFin</Text>
          </View>
          <View style={estilosHome.cabecalhoDireita}>
            <TouchableOpacity style={estilosHome.botaoEngrenagem} onPress={() => router.push('/(tabs)/Configuracoes')}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 140 }}>
          ...
          <Text style={[estilosHome.saldo, { color: saldoAtual >= 0 ? CORES.verde : CORES.vermelho }]} numberOfLines={1} adjustsFontSizeToFit>
          ...
          {listaFiltrada.slice(0, 10).map((item) => {
            const cor = item.tipo === 'receita' ? CORES.verde : item.ehMeta ? '#FACC15' : CORES.vermelho;
            return (
              <View style={estilosHome.itemTransacao} key={item.id}>
                <View style={estilosHome.iconeTransacao}>
                  <Ionicons
                    name={item.tipo === 'receita' ? 'wallet-outline' : item.ehMeta ? 'flag-outline' : 'arrow-down-outline'}
                    size={20} color={cor}
                  />
                </View>
                <View style={{ flex: 1 }}>
                ...
                <Text style={[estilosHome.valorTransacao, { color: cor }]} numberOfLines={1} adjustsFontSizeToFit>
        ...
        <PerfilModal visible={modalPerfil} onClose={() => setModalPerfil(false)}
          nome={nomeUsuario} email={emailUsuario} usuarioId={usuarioId} onSair={sairDaConta} />
              ...
              <ScrollView keyboardShouldPersistTaps="handled">
                <View style={estilosHome.conteudoModal}>
                  ...
                  <View style={estilosHome.filhaTipo}>
                    ...
                    >
                      <Ionicons name="arrow-up-outline" size={16} color={tipo === 'receita' ? '#fff' : CORES.textoMedio} style={{ marginBottom: 2 }} />
                    ...
                    >
                      <Ionicons name="arrow-down-outline" size={16} color={tipo === 'despesa' ? '#fff' : CORES.textoMedio} style={{ marginBottom: 2 }} />
                  ...
                  {tipo === 'despesa' && (
                    <View style={estilosHome.secaoMeta}>
                      ...
                      {/* Dois botões bem grandes e claros */}
                      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
                        <TouchableOpacity
                          style={[
                            estilosHome.botaoTipoMeta,
                            !ehDespesaMeta && { backgroundColor: CORES.vermelho + '20', borderColor: CORES.vermelho },
                          ]}
                          onPress={() => { setEhDespesaMeta(false); setMetaSelecionada(null); }}
                        >
                          <Text style={{ fontSize: 22, marginBottom: 4 }}>🔴</Text>
                          <Text style={[estilosHome.textoTipoMeta, !ehDespesaMeta && { color: CORES.vermelho, fontWeight: 'bold' }]}>
                            Normal
                          </Text>
                        ...
                        <TouchableOpacity
                          style={[
                            estilosHome.botaoTipoMeta,
                            ehDespesaMeta && { backgroundColor: '#FACC1520', borderColor: '#FACC15' },
                          ]}
                          onPress={() => setEhDespesaMeta(true)}
                        >
                          <Text style={{ fontSize: 22, marginBottom: 4 }}>🎯</Text>
                          <Text style={[estilosHome.textoTipoMeta, ehDespesaMeta && { color: '#FACC15', fontWeight: 'bold' }]}>
                            Para meta
                          </Text>
                                ...
                                return (
                                  <TouchableOpacity
                                    ...
                                    style={[
                                      ...
                                      selecionada && { borderColor: '#FACC15', backgroundColor: '#FACC1510' },
                                    ]}
                                    onPress={() => setMetaSelecionada(meta)}
                                  >
                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                                      <Ionicons name="flag" size={14} color="#FACC15" style={{ marginRight: 6 }} />
                                      <Text style={[
                                        estilosHome.textoMetaOpcao,
                                        selecionada && { color: '#FACC15', fontWeight: 'bold' },
                                      ]}>
                                        {meta.nome}
                                      </Text>
                                      {selecionada && (
                                        <Ionicons name="checkmark-circle" size={16} color="#FACC15" style={{ marginLeft: 'auto' }} />
                                      )}
                                    </View>
                                    {/* Mini barra de progresso */}
                                    <View style={estilosHome.miniBarraFundo}>
                                      <View style={[estilosHome.miniBarraProgresso, { width: `${progresso}%` as any }]} />
                                    </View>
                                    <Text style={estilosHome.miniBarraTexto}>
                                      {progresso.toFixed(0)}% de {meta.valorAlvo.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                    </Text>
                                  </TouchableOpacity>
                                );
                              })}
                  ...
                  <View style={estilosHome.filhaBotoes}>
                    ...
                    <TouchableOpacity style={[estilosHome.botaoSalvar, salvando && { opacity: 0.7 }]} onPress={salvarTransacao} disabled={salvando}>
